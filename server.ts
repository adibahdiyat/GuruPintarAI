      ...
      try {
        ...
        if (
          ...
          if (
            ...
            combinedMsg.includes("api_key_invalid") || 
            combinedMsg.includes("api key invalid") || 
            combinedMsg.includes("invalid key") ||
            combinedMsg.includes("key is not valid")
          ) {
            throw new Error("Kunci API (API Key) Google Gemini tidak valid atau tidak memiliki akses aktif. Silakan buka menu 'Profil Saya' dan ganti dengan API Key yang valid dari Google AI Studio.");
          }
          throw err;
        }
...
- Refleksi Guru (pertanyaan evaluasi diri guru)
      ...
      const response = await generateContentWithRetryAndFallback(ai, {
        contents: { parts },
        config: {
          systemInstruction: `Anda adalah pakar kurikulum Kurikulum Merdeka 2026 pendidikan nasional di Indonesia, desainer UI/UX asisten digital guru tepercaya, dan desainer instruksional ahli. Semua materi, RPP, LKPD, dan Asesmen harus diulas lengkap, panjang, sangat memuaskan, mendalam, realistis, dan langsung siap saji. DILARANG KERAS menggunakan tag/format HTML dalam bentuk apa pun dan DILARANG KERAS menggunakan tanda bintang (asterisk '**' atau '*') untuk mempertebal kata/kalimat. Untuk penekanan atau judul/subjudul, gunakan huruf KAPITAL (UPPERCASE) atau heading standard (#, ##, ###) tanpa tanda bintang di sekelilingnya. Sifat responsmu wajib berwujud JSON murni yang sangat detail, panjang, komprehensif, dan siap diekspor tanpa tambahan teks markdown pembuka/penutup (tanpa \`\`\`json).`,
...
- Refleksi Guru (pertanyaan evaluasi diri guru)
      ...
      const response = await generateContentWithRetryAndFallback(ai, {
        ...
        config: {
          systemInstruction: "Anda adalah pakar kurikulum nasional di Indonesia (Kurikulum Merdeka 2026). Semua materi, RPP, LKPD, dan Asesmen harus diulas lengkap, panjang, sangat memuaskan, mendalam, realistis, dan langsung siap saji. DILARANG KERAS menggunakan tag/format HTML dalam bentuk apa pun dan DILARANG KERAS menggunakan tanda bintang (asterisk '**' atau '*') untuk mempertebal kata/kalimat. Untuk penekanan atau judul/subjudul, gunakan huruf KAPITAL (UPPERCASE) atau heading standard (#, ##, ###) tanpa tanda bintang di sekelilingnya. Kembalikan JSON murni sesuai schema yang kaku tanpa potongan markdown (tanpa ```json).",
          responseMimeType: "application/json",
          responseSchema: {
            type: Type.OBJECT,
            properties: {
...
4. FORMATTING WAJIB: DILARANG menggunakan tag HTML apa pun (seperti <b>, <strong>, <i>, <br>, dll.) dan DILARANG menggunakan tanda bintang (asterisk '**' atau '*') untuk mempertebal kata/kalimat. Untuk subjudul atau penekanan, gunakan huruf KAPITAL (UPPERCASE) atau heading standard (#, ##, ###) tanpa tanda bintang di sekelilingnya. Untuk poin list, gunakan tanda hubung (-) atau penomoran biasa (1., a.). Hal ini agar dokumen bersih dan bisa langsung disalin atau diedit dengan nyaman di Microsoft Word (DOCX).
`;
...
2. Jawablah secara terperinci, utuh, tanpa kode singkatan/titik-titik.
3. Gunakan bahasa yang profesional, adaptif, namun mudah dipahami oleh guru di Indonesia.
4. FORMATTING WAJIB: DILARANG menggunakan tag HTML apa pun (seperti <b>, <strong>, <i>, <br>, dll.) dan DILARANG menggunakan tanda bintang (asterisk '**' or '*') untuk mempertebal kata/kalimat. Untuk subjudul atau penekanan, gunakan huruf KAPITAL (UPPERCASE) atau heading standard (#, ##, ###) tanpa tanda bintang di sekelilingnya. Untuk poin list, gunakan tanda hubung (-) atau penomoran biasa (1., a.). Hal ini agar dokumen bersih dan bisa langsung disalin atau diedit dengan nyaman di Microsoft Word (DOCX).
`;
...
Semua isi harus lengkap, tidak ada placeholder atau titik-titik.`;
      ...
      const response = await generateContentWithRetryAndFallback(ai, {
        contents: prompt,
        config: {
          systemInstruction: "Anda adalah pakar kurikulum Kurikulum Merdeka 2026. Buat TP dan ATP yang detail, lengkap, terukur, dan siap digunakan guru. Dilarang menggunakan placeholder atau titik-titik. FORMATTING WAJIB: DILARANG menggunakan tag HTML apa pun dan DILARANG menggunakan tanda bintang (asterisk '**' atau '*') untuk mempertebal kata. Untuk subjudul atau penekanan, gunakan huruf KAPITAL (UPPERCASE) atau heading standard (#, ##, ###) tanpa tanda bintang di sekelilingnya. Gunakan tanda hubung (-) atau penomoran biasa untuk list. Hal ini agar dokumen bisa langsung disalin atau diedit dengan nyaman di Word (DOCX).",
        }
      });
...
Semua soal harus kontekstual, sesuai level kelas, dan tidak ada yang kosong.`;
      ...
      const response = await generateContentWithRetryAndFallback(ai, {
        contents: prompt,
        config: {
          systemInstruction: "Anda adalah pembuat soal ahli Kurikulum Merdeka 2026. Buat soal ujian yang berkualitas, sesuai level kognitif Bloom, tanpa placeholder, lengkap dengan kunci dan pembahasan. FORMATTING WAJIB: DILARANG menggunakan tag HTML apa pun dan DILARANG menggunakan tanda bintang (asterisk '**' atau '*') untuk mempertebal kata. Untuk subjudul atau penekanan, gunakan huruf KAPITAL (UPPERCASE) atau heading standard (#, ##, ###) tanpa tanda bintang di sekelilingnya. Gunakan tanda hubung (-) atau penomoran biasa untuk list. Hal ini agar dokumen bisa langsung disalin atau diedit dengan nyaman di Word (DOCX).",
        }
      });
...
Tulis semua siswa tanpa terkecuali.`;
      ...
      const response = await generateContentWithRetryAndFallback(ai, {
        contents: prompt,
        config: { systemInstruction: "Tulis narasi rapor yang hangat, personal, dan berbeda untuk setiap siswa. Jangan seragam. FORMATTING WAJIB: DILARANG menggunakan tag HTML apa pun dan DILARANG menggunakan tanda bintang (asterisk '**' atau '*') untuk mempertebal kata. Untuk subjudul atau penekanan, gunakan huruf KAPITAL (UPPERCASE) atau heading standard (#, ##, ###) tanpa tanda bintang di sekelilingnya." }
      });
...
DILARANG KERAS menggunakan istilah keagamaan, nama-nama yang mengarah ke agama tertentu secara spesifik (misal: nuansa Islam seperti santri, ustadz, masjid, dll), atau berasumsi tentang keyakinan tertentu. Semua soal harus sepenuhnya netral dan profesional.`}`;
      ...
      const response = await generateContentWithRetryAndFallback(ai, {
        contents: prompt,
        config: { systemInstruction: "Buat soal kuis yang menarik, kontekstual, netral-sekolah, mudah dicerna, sederhana, cocok dengan kapasitas kognitif tingkat kelas yang dituju, dan ramah anak. Jawaban harus jelas dengan penjelasan singkat. FORMATTING WAJIB: DILARANG menggunakan tag HTML apa pun dan DILARANG menggunakan tanda bintang (asterisk '**' atau '*') untuk mempertebal kata. Untuk subjudul atau penekanan, gunakan huruf KAPITAL (UPPERCASE) atau heading standard (#, ##, ###) tanpa tanda bintang di sekelilingnya. Gunakan tanda hubung (-) atau penomoran biasa untuk list." }
      });
